...
#' @param sigma.use Diffusion scale parameter of the Gaussian kernel. One of '\code{local}',
...
#' @param knn Number of nearest neighbors to consider (default: a guess betweeen 100 and n - 1.
#'    See \code{\link[destiny]{find_dm_k}}).
...
runDiffusionMap <- function(object, sigma.use = NULL,
                            ...
                            density.norm = TRUE, knn = NULL, verbose=T,
                            ...) {

  dm.data <- as.matrix(object@gate.data)
  ...
  if (is.null(sigma.use)) {
    ...
    sigma.use <- find_sigmas(dm.data, verbose=F)@optimal_sigma
    message(paste0(Sys.time(), " [WARNING] Invalid sigma value. Using an optimal global sigma instead."))
  }

  # Figure out k-nearest neighbors
  if (is.null(knn)) {
    knn <- find_dm_k(nrow(dm.data))
    if (verbose) message(paste0(Sys.time(), " [INFO] Using ", knn, " nearest neighbors."))
  }

  # Calculate the Diffusion Map
  dm.obj <- DiffusionMap(dm.data, sigma=sigma.use, k=knn, density_norm = density.norm, distance=distance[1], ...)
