...
exp.data <- runExprsExtract(fcs.file, showDesc = FALSE)
...
set.seed(1)
fspy <- runCluster(fspy, cluster.method = "som", xdim = 9, ydim = 9, verbose = T)
...
fspy <- processingCluster(fspy, downsampleing.size = 0.02, force.resample = F)

# Now there is only 23,663 cells enrolled in the dimensionality reduction
fspy
...
fspy <- defRootCells(fspy, root.cells = c(26), verbose = T)
...
# Tree plot
plotTree(fspy, color.by = "pseudotime", show.node.name = T, cex.size = 1) + 
